...
export default class NodeSearchProvider implements ISearchProvider {
  ...
  renderSearchItem(node: Node): React.ReactNode {
    ...
    return (
      <WFace.WListItem
        ...
        dense
        key={"key-search-item-" + node.Id}
      >
        <WFace.WListItemIcon>{implementation.getIcon()}</WFace.WListItemIcon>
        <WFace.WListItemText
          primary={node.Name}
          secondary={node.Labels.join(", ")}
        />
      </WFace.WListItem>
    );
  }
  ...
  onItemSelected(node: Node) {
    if (!this.nodeBrowserScreen) {
      this.nodeBrowserScreen = MenuTreeUtil.findByName(
        this.appContext.menuTree,
        "NodeBrowser"
      );
    }
    ...
    if (this.nodeBrowserScreen) {
      const implementation = getNodeImplementation(node);
      this.openScreen({
        ...this.nodeBrowserScreen,
        initialValues: { node: implementation },
      });
    }
  }
}
